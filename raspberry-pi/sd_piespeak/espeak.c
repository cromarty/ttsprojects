...
static int synth_callback(short *wav, int numsamples, espeak_EVENT *events) {
		...
		case espeakEVENT_PLAY:
			{
				...
				send_audio_upto(st, wav, &numsamples_sent, upto);
				break;
			}
		default:
			break;
	...
	send_audio_upto(st, wav, &numsamples_sent, numsamples);
...
static uint32_t pbq_add_audio(TTSRENDER_STATE_T *st, short *audio_chunk, int num_samples) {
	pthread_mutex_lock(&st->pbq_mutex);
	while (!st->stop_requested && queue_size(st->pbq) > PBQ_MAX_SIZE) {
		pthread_cond_wait(&st->pbq_cv, &st->pbq_mutex);
	}
	pthread_mutex_unlock(&st->pbq_mutex);
	if (st->stop_requested) {
	...
	queue_enqueue(&st->pbq, (void*)entry);
...
static void delete_pbq_entry(TTSRENDER_STATE_T *st) {
	...
	queue_dequeue(&st->pbq, (void*)pbqentry);
...
static void clear_pbq(TTSRENDER_STATE_T *st, ) {
	pthread_mutex_lock(&st->pbq_mutex);
	while (queue_size(&st->pbq) > 0 ) {
		delete_pbq_entry(st);
	}
	queue_destroy(st->pbq);
	pthread_mutex_unlock(&st->pbq_mutex);
} // end clear_pbq


static uint32_t send_audio_upto(
	TTSRENDER_STATE_STATE_T *st,
	short *wav,
	int *sent,
	int upto)
{
	...
	result = pbq_add_audio(st, start, numsamples);
	*sent = upto;
	return result;
} // end send_audio_upto
