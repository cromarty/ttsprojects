...
static espeak_ERROR speak_text(struct synth_t *s)
{
	...
	rc = espeak_Synth(s->buf, s->len + 1, 0, POS_CHARACTER, 0, synth_mode,NULL, st);
	return rc;
}
...
/* create an ilctts status object */
	res = ilctts_create(&st, 22050, 1, 16, ILC_BUF_COUNT, BUF_SIZE_MS, 0, (1024*6));
	if (res == -1) {
		fprintf(stderr, "Unable to initialize ilctts.\n");
		return -1;
	}
	...
	if (res == -1) {
		fprintf(stderr, "Failed to start the ilctts ring buffer consumer thread.\n");
		return -1;
	}
